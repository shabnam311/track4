            ...
            <div className={\`max-w-[85%] p-4 rounded-2xl shadow-sm \${
              ...
              {m.type === 'image' && (
                <div className="flex flex-col">
                  {m.imageBase64 && (
                    <img src={m.imageBase64} alt="Uploaded by user" className="max-w-full rounded-xl mb-2 border border-black/5 dark:border-white/5" />
                  )}
                  {m.text && <p>{m.text}</p>}
                </div>
              )}
      ...
      <div className="bg-white dark:bg-soil-800 p-4 border-t border-black/10 dark:border-white/10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
        {selectedImage && (
          <div className="mb-3 relative inline-block">
            <img src={selectedImage} alt="Preview" className="h-16 w-16 object-cover rounded-xl border border-black/10 dark:border-white/10" />
            <button 
