...
const LangBtn = ({ code, label, currentLang, changeLanguage }) => (
  <button onClick={() => changeLanguage(code)} className={`px-2 py-1 rounded text-xs font-bold border border-black/15 dark:border-white/10 transition ${currentLang === code ? 'bg-soil-900 dark:bg-soil-800 text-wheat-100 border-soil-900 dark:border-white/20' : 'bg-transparent text-soil-700 dark:text-wheat-400 hover:bg-black/5 dark:hover:bg-white/5'}`}>{label}</button>
);
    ...
    <div className="min-h-screen bg-paper dark:bg-soil-900 text-soil-900 dark:text-wheat-100 bg-radial-pattern font-sans flex flex-col">
      <header className="flex flex-col md:flex-row justify-between items-center px-6 py-5 md:px-12 md:py-6 gap-4">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          ...
          <LangBtn code="hi" label="HI" currentLang={i18n.language} changeLanguage={changeLanguage} />
          <LangBtn code="ta" label="TA" currentLang={i18n.language} changeLanguage={changeLanguage} />
          <LangBtn code="mr" label="MR" currentLang={i18n.language} changeLanguage={changeLanguage} />
          <LangBtn code="pa" label="PA" currentLang={i18n.language} changeLanguage={changeLanguage} />
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-col gap-3 max-w-sm">
            <div className="flex gap-2">
              ...
              >
                {t('farmer_anjali') || 'Farmer: Anjali (MP)'} <span><ArrowRight className="w-4 h-4 inline" /></span>
              </button>
              <button 
              ...
              >
                {t('farmer_karthik') || 'Karthik (TN)'} <span><ArrowRight className="w-4 h-4 inline" /></span>
              </button>
            </div>
            <button 
            ...
            >
              Continue as Policymaker <span><ArrowRight className="w-4 h-4 inline" /></span>
            </button>
            <button 
