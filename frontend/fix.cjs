  ...
  if (content.includes('JSON.parse(localStorage.getItem')) {
    ...
    if (!content.includes('const safeJSONParse')) {
      ...
      for(let i=0; i<parts.length; i++) {
        if(parts[i].startsWith('import ')) importEnd = i;
      }
      parts.splice(importEnd + 1, 0, safeParseCode);
      content = parts.join('\n');
    }
  }

  if (path.includes('FarmerHome')) {
    content = content.replace("farmer.name.substring(0, 2)", "(farmer?.name || 'AN').substring(0, 2)");
    content = content.replace("farmer.name}", "(farmer?.name || 'Anjali')}");
  }

  // Remove corrupted chars in App.jsx
